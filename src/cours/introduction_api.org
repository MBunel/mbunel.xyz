...
Le [[https://gitlab.huma-num.fr/mbunel/formation-apis/-/raw/main/Support/diapo.pdf?ref_type=heads][supporte de formation]] associé, le [[https://gitlab.huma-num.fr/mbunel/formation-apis/-/blob/main/TP/TP_api.ipynb?ref_type=heads][notebook/ du TP]] et les [[https://gitlab.huma-num.fr/mbunel/formation-apis][sources du projet]] sont disponibles sous les termes de la licence [[https://creativecommons.org/licenses/by-sa/4.0/deed.fr][CC-BY-SA 4.0]] (Attribution — Partage dans les mêmes conditions).
#+END_abstract


Ce notebook décrit un exemple d'utilisation chaînée de trois API. L'objectif est de présenter différentes API utiles et de montrer le travail de "plomberie" qu'il est parfois nécessaire d'effectuer pour combiner différentes API.
...
L'objectif de ce TP est de construire le profil de l'itinéraire de déplacement entre deux lieux dont on ne connaît que le toponyme.

Les étapes sont les suivantes:

1. Géocodage
   1. Identifier les coordonnées du point de départ, le /pré de madame Carles/
   2. Identifier les coordonnées du point d'arrivée, le /refuge des Écrins/
...
#+begin_src python
# le point d'exclamation (!) qui précède la commande *pip* indique que le contenu
...
On importe ensuite les bibliothèques nécessaires à l'extraction des données :
...
- =curlify= : Facultatif, utilisé ici pour construire l'équivalent
...
Dans cette première étape on va utiliser l'API de géocodage de la Base adresse nationale (BAN) pour récupérer les coordonnées d'un lieu.
...
Comme on peut le voir, cette structure de données contient beaucoup d'informations. Or, la seule qui nous intéresse réellement ici sont les coordonnées de la géométrie retournée.

Si l'on observe attentivement la réponse, on peut trouver les coordonnées de la géométrie sous la forme d'une liste de deux valeurs. Cette liste est associée à la clé =coordinates=. L'élément =coordinates= est lui-même inclus dans un objet, associé à la clé =geometry=. Enfin, l'objet contenant la clé =geometry= est inclus dans une liste, elle-même
associée à la clé =feature= d'un dernier objet.

Le chemin de nos coordonnées est donc : =features > 0 > geometry > coordinates=. Valeur que l'on pourra récupérer avec la syntaxe suivante :
...
On peut alors vérifier que la variable =corrd_pre= nouvellement créée contient bien le résultat attendu, les coordonnées du lieu.
...
On peut vérifier que l'on dispose bien de coordonnées pour ce second lieu.
...
Au terme de cette première partie on dispose donc de deux informations, les coordonnées de deux lieux distincts, chacun récupérés à l'aide d'une requête à l'API de Géocodage de la BAN.

** Calcul itinéraire

Dans cette seconde partie on va utiliser une nouvelle API, un peu plus compliquée, pour calculer l'itinéraire entre les deux coordonnées précédemment calculées.
...
#+begin_src python
payload = {
    # On transforme la liste des coordonnées en une liste de valeurs séparées
    # par des pipes ("|")
...
# Cette fois, les paramètres sont transmis à l'aide de l'option "json="
...
On peut donc reconstruire une polyligne à partir de ces trois listes.
...
#+begin_src python
# Shapely est une bibliothèque qui permet d'interagir plus simplement avec OGR (traitement des géométries vectorielles)
import shapely
#+end_src
...
#+begin_src python
# On récupére le centroïde de la ligne (sans son Z)
...
#+begin_src python
# On crée une carte, centrée sur le centroïde de la ligne
