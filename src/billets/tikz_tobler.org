...
#+bibliography: ../meta/biblio_other.bib 
#+FILETAGS: :tikz:

Dans un article de 1993 [cite/author:@tobler1993] a proposé un modèle de marche basé sur des données empiriques de [cite:@imhof1950].

La figure suivante est une reproduction en =tikz= de la figure originelle.
...
La figure a été générée avec le code suivant :

#+HEADER: :headers '("\\usepackage{pgfplots}" "\\usepackage{siunitx}")
#+HEADER: :imagemagick yes
#+HEADER: :results none :exports both
...
#+HEADER: :headers '("\\usepackage{pgfplots}" "\\usepackage{xcolor}" "\\usepackage{siunitx}")
#+HEADER: :imagemagick yes
#+HEADER: :results none
